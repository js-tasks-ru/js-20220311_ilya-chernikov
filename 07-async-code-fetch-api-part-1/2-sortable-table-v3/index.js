...
export default class SortableTable {
  ...
  sortOnServer (fieldValue, orderValue) {
    this.start = 0;
    this.end = CHUNK_VALUE;
    this.fetchData(orderValue, fieldValue).then(data => this._replaceData(data));
  }
  ...
  fetchData (order = this.defaultSort.order, field = this.defaultSort.id) {
    this.elementBodyLink.parentElement.classList.add('sortable-table_loading');
